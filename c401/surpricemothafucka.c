...
if(RootPtr != NULL)
    ...
    return FALSE;		  /* V pøípadì øe¹ení sma¾te tento øádek! */
...
if(RootPtr == NULL) //neni nic poslano
        return ;
    if(*RootPtr == NULL)
    {
        tBSTNodePtr myNode = malloc(sizeof(struct tBSTNode));
        if(myNode == NULL)
            return;
            else {
        myNode->Key         = K;
        myNode->BSTNodeCont = Content;
        myNode->LPtr        = NULL;
        myNode->RPtr        = NULL;
        *RootPtr = myNode;
        }
...
**/
    ...
    {
    tBSTNodePtr myNode = NULL;


    if((*RootPtr)->RPtr == NULL)
    {
        myNode               = *RootPtr;
        PtrReplaced->Key         = myNode->Key;
        PtrReplaced->BSTNodeCont = myNode->BSTNodeCont;
        *RootPtr                 = myNode->LPtr;
        free(myNode);
        return ;
    }
...
**/
    ...
    {   if((*RootPtr)->RPtr != NULL) //pokud mohu jit doleva
        {
            BSTDispose(&(*RootPtr)->RPtr);
        }
        if((*RootPtr)->LPtr != NULL) //pokud mohu jit doleva
        {
